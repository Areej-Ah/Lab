...
class AttributeOptionDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addColumn('checkbox', 'admin.attribute_options.btn.checkbox')
            ->addColumn('edit', 'admin.attribute_options.btn.edit')
            ->addColumn('delete', 'admin.attribute_options.btn.delete')
            ->rawColumns([
                'edit',
                'delete',
                'checkbox',
     ...
     *
     * @param \App\Model\AttributeOption $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query()
    {
        return AttributeOption::query()->with('attribute')->select('attribute_options.*');
    }
    ...
    {
        return $this->builder()
                    ->setTableId('AttributeOptiondatatable-table')
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('code')
            ->title(trans('admin.code_attr'))
            ->data('code')
            ->name('code'),


            Column::make('attribute_id')
            ->title(trans('admin.attribute_id'))
            ->data('attribute.name_'.session('lang'))
            ->name('attribute.name_'.session('lang')),
    ...
    {
        return 'AttributeOption_' . date('YmdHis');
    }
}
